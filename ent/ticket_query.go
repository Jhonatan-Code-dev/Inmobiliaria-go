...
func (_q *TicketQuery) QueryEmpresa() *EmpresaQuery {
...
func (_q *TicketQuery) QueryUnidad() *UnidadQuery {
...
func (_q *TicketQuery) QueryCliente() *ClienteQuery {
...
func (_q *TicketQuery) WithEmpresa(opts ...func(*EmpresaQuery)) *TicketQuery {
	query := (&EmpresaClient{config: _q.config}).Query()
...
func (_q *TicketQuery) WithUnidad(opts ...func(*UnidadQuery)) *TicketQuery {
	query := (&UnidadClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TicketQuery) WithCliente(opts ...func(*ClienteQuery)) *TicketQuery {
	query := (&ClienteClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TicketQuery) loadEmpresa(ctx context.Context, query *EmpresaQuery, nodes []*Ticket, init func(*Ticket), assign func(*Ticket, *Empresa)) error {
...
func (_q *TicketQuery) loadUnidad(ctx context.Context, query *UnidadQuery, nodes []*Ticket, init func(*Ticket), assign func(*Ticket, *Unidad)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Ticket)
	for i := range nodes {
		fk := nodes[i].UnidadID
...
func (_q *TicketQuery) loadCliente(ctx context.Context, query *ClienteQuery, nodes []*Ticket, init func(*Ticket), assign func(*Ticket, *Cliente)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Ticket)
	for i := range nodes {
...
func (_q *TicketQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withEmpresa != nil {
			_spec.Node.AddColumnOnce(ticket.FieldEmpresaID)
		}
		if _q.withUnidad != nil {
			_spec.Node.AddColumnOnce(ticket.FieldUnidadID)
		}
		if _q.withCliente != nil {
			_spec.Node.AddColumnOnce(ticket.FieldClienteID)
		}
	}
